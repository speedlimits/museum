        ...
        )
{
	...
	#define NWAVES 2
	Wave wave[NWAVES] = {
		{ 1.0, 1.0, 0.5, float2(-1, 0),.5 },
		{ 2.0, 0.5, 1.7, float2(-0.7, 0.7),.5*.7 }
	};
	...
	// wave synthesis using two sine waves at different frequencies and phase shift
        float2 worldpos=mul(World,P).xz;
        float distance=length(worldpos);
        float divisor=(distance-5000)/(.5*.7*1000);
        float divisorb=sqrt((distance-5000)/1000);
        float angle_time[2]={dot(wave[0].dir, worldpos/8),distance/divisor};
	for(int i = 0; i<NWAVES; ++i)
	{
		angle = (angle_time[i]) * wave[i].freq + time * wave[i].phase;
