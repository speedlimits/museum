...
/*
    vec3 lightnorm;
    float tim=2*3.1415926536*timex.x/128.0;
    lightnorm.y=-cos(tim);
    lightnorm.x=.86*sin(tim);
    lightnorm.z=.5*sin(tim);
  */  
	...
	vec3 P = gl_Vertex.xyz - camPosLocal.xyz+vec3(0,500,0);
    //max_y*=clamp(lightnorm.y,.125,.0);    
	// Set altitude
	if (P.y > 0.)
	{
		P *= (max_y.x / P.y);
	...
	temp2.x *= glow.x*sqrt(clamp(abs(lightnorm.y),0.05,1.0));
